...
/**
 * Class that creats mealplans 
 * and stores the stores, recipes, and ingredients available.
 * 2nd version that tries to minimize ingredients too
 */
public class MealPlannerMinIngredients {
     ...
     */
	public MealPlannerMinIngredients(Collection<Recipe> recipesKnown,
	...
	/**
     * Finds an optimal MealPlan with numMeals recipes starting with a random one
     *
	...
	private MealPlan makePlan(int numMeals, ArrayList<Recipe> forced) {
		...
		while (modForced.size() > 0 && used < numMeals) {
			...
			for (Recipe r : modForced) {
				// Sorts by least stores needed and least ingredients needed, makes sure items in store
	...
	/**
     * Finds the ingredients needed for a recipe
     *
	...
	/**
     * Finds the weight of a recipe by multiplying the number of new stores by new ingredients.
     *
     * @param recipe recipe to check
     * @param ingredients ingredients available
     * 
     * @return the number of needed stores if ingredients present in stores multiplied by number of new ingredients needed
	...
	private int recipeOptimal(Recipe recipe, Collection<Ingredient> using, Collection<Ingredient> ingredients) {
		...
		// Finds new ingredients needed
		ArrayList<Ingredient> modNeeded = new ArrayList<>(needed);
		for (Ingredient i : using) {
			for (Ingredient j : needed) {
		...
		// returns the number of stores needed multiplied by new ingredients needed
		return ourStores.keySet().size() + modNeeded.size();
